...
class Booking(models.Model):
    ...
    # факт свободности аудитории в календаре 8 лаба
    FREE_AUDIENCES = (
        (0, 'Свободная'),
        (1, 'Занята')
    )
    audiences = models.ManyToManyField(Audiences, verbose_name="Аудитории", null=True)
    name = models.CharField(verbose_name="Номер", default="Имя заявки")
    description = models.TextField(verbose_name="Описание", default="Описание заявки", blank=True, null=True)
    ...
    owner = models.ForeignKey(CustomUser, on_delete=models.CASCADE, verbose_name="Покупатель", related_name='owner', null=False)
