...
def search_audiences(request):
    ...
    audiences = Audiences.objects.filter(status=1,name__contains=name)
...
def add_audiences_to_booking(request, audiences_id):
    ...
    if not Audiences.objects.filter(pk=audiences_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)
    audience = Audiences.objects.get(pk=audiences_id)
    # print(audience)
    
    booking = Booking.objects.filter(status=1, owner__id__exact=user_id).last()
    # print(booking)
    
    if booking is None:
        booking = Booking.objects.create(owner=CustomUser.objects.get(pk=user_id))
    # audiencess = Audiences.objects.filter(status=1)
    booking.audiences.add(audience)
...
def get_booking(request):
    ...
    user_id = payload["user_id"]
    bookings = Booking.objects.filter(owner__id__exact=user_id)
...
def update_status_user(request, booking_id):
    ...
    if request_status not in [1,2, 5]:
        return Response(status=status.HTTP_405_METHOD_NOT_ALLOWED)

    bookings = Booking.objects.get(pk=booking_id)
    old_status = bookings.status
...
def delete_audiences_from_booking(request, booking_id, audiences_id):
    ...
    booking = Booking.objects.get(pk=booking_id)
    booking.audiences.remove(Audiences.objects.get(pk=audiences_id))
    booking.save()
...
@api_view(["DELETE"])
@permission_classes([IsAuthenticated])
def delete_substance_from_cosmetic(request, booking_id, audiences_id):
    if not BookingAudiences.objects.filter(booking_id=booking_id, audiences_id=audiences_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    item = BookingAudiences.objects.get(booking_id=booking_id, audiences_id=audiences_id)
    item.delete()
...
#переделать под свое что за процентное соотншение
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_substance_in_cosmetic(request, cosmetic_id, substance_id):
    if not BookingAudiences.objects.filter(substance_id=substance_id, cosmetic_id=cosmetic_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    item = BookingAudiences.objects.get(substance_id=substance_id, cosmetic_id=cosmetic_id)
    return Response(item.percent_in)


@api_view(["PUT"])
@permission_classes([IsAuthenticated])
def update_substance_in_cosmetic(request, booking_id, audiences_id):
    if not BookingAudiences.objects.filter(audiences_id=audiences_id, booking_id=booking_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    item = BookingAudiences.objects.get(audiences_id=audiences_id, booking_id=booking_id)

    serializer = BookingAudiencesSerializer(item, data=request.data, many=False, partial=True)
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_draft_booking(request):
    token = get_access_token(request)
    payload = get_jwt_payload(token)
    user_id = payload["user_id"]
    print('11111111111!!!!!!!!!!!!!!!!!!!111111111111111111')
    print(user_id)
    order = Booking.objects.filter(owner__id__exact=user_id, status=1).last()
    print(order)
    if order is None:
        return Response(status=status.HTTP_404_NOT_FOUND)
