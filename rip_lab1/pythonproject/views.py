...
def GetRoom(request, id):
    audience = next((sub for sub in room_arr if sub["id"] == id), None)
    if audience:
        print(audience["title"])
    else:
        print("Not found!")
    return render(request, 'audience.html', {'data' : {
        'current_date': date.today(),
        'id': id,
        'audiences': audience,
    }})
...
def GetRoomSearch(request):
    ...
    for i in room_arr:
        if input_text is not None:
            ...
            return render(request, 'audiences.html', {'data' : {
                'audiences': room_arr,
                'query': input_text,
            }})
    return render(request, 'audiences.html', {'data' : {
